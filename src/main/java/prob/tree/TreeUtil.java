...
public class TreeUtil {
	...
	/**
	* Print tree given height of tree given root node.
	**/
	...
	private static void printTree(final Node <Character> root, final char ch[][], final int h, final int x, final int y) {
		if (root == null)
			return;
		ch[x][y] = root.data;
	...
	/**
	* Find height of a tree given root node.
	**/
	public static <T extends Comparable<T>> int heightTree(final Node <T> root) {
		if (root == null)
			return 0;
	...
	/**
	* Do an inorder traversal of tree given root node.
	**/
	public static <T extends Comparable<T>> void inOrder(final Node <T> root, final List <T> result) {
	...
	/**
	* Create a tree using pre order traversal, assuming in-order traversal is sorted character.
	**/
